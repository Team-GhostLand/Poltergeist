...
const autoCompleteHandler = async (interaction: AutocompleteInteraction, client: Bot) => {
...
export async function run(client: Bot, interaction: Interaction) {
    if (interaction.isCommand()) return commandHandler(interaction as ChatInputCommandInteraction, client);
    if (interaction.isAutocomplete()) return autoCompleteHandler(interaction as AutocompleteInteraction, client);
